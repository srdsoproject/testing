...
================================================
Single entry-point that contains BOTH original modules unchanged:

  1) GENERAL ANALYSIS (PIL-based summary dashboards)
     – from master_code.py / general_analysis_dashboards.py
     – functions: generate_general, generate_all_general, create_dashboard, ...

  2) DETAILED DASHBOARDS (matplotlib jurisdiction / classification)
     – from combinations.py / railway_safety_dashboards.py
     – functions: generate_elect_g, generate_engineering, generate_elect_trd,
                  generate_mechanical, generate_operating, generate_commercial,
                  generate_elect_tro, generate_snt, generate_all, ...

No logic, classification rules, filters, colours, layouts or output names
have been altered. Both families produce the same files they produced when
run as separate scripts.

Usage
-----
    # Everything (all detailed + all general)
    python railway_safety_unified.py
    python railway_safety_unified.py all

    # Only detailed family (same as old combinations.py)
    python railway_safety_unified.py detailed
    python railway_safety_unified.py elect_g engg operating

    # Only general / PIL family (same as old master_code.py)
    python railway_safety_unified.py general
    python railway_safety_unified.py general operating commercial engg

    # Mixed – any key from either family
    python railway_safety_unified.py elect_g general_operating engg_full

From Python:
    from railway_safety_unified import generate_all, generate_all_general, generate_all_detailed
    generate_all()                 # detailed only (CONFIG flags)
    generate_all_general()         # all PIL general dashboards
    generate_all_detailed()        # all detailed (ignores GENERATE flags)
"""
...
# ---------- GENERAL (PIL) family settings ----------
PERIOD_TEXT = "PERFORMANCE PERIOD: April to July 2026"
W = 1600

ENGINEERING_TARGETS = [None, "Sr.DEN/C", "Sr.DEN/S", "DEN/TRACK"]
...
DEPARTMENT_DISPLAY_NAMES = {
    ...
    "ELECT/G": "ELECT/G DEPARTMENT",
    "ELECT/TRO": "ELECT/TRO DEPARTMENT",
    "ELECT/TRD": "ELECT/TRD DEPARTMENT",
    "OPTG": "OPERATING DEPARTMENT",
    "COMMERCIAL": "COMMERCIAL DEPARTMENT",
    "MECHANICAL": "MECHANICAL DEPARTMENT",
    "SIGNAL & TELECOM": "SIGNAL & TELECOM DEPARTMENT",
}

KEY_MAP = {
    "engg": ("ENGINEERING", "Sr.DEN/C"),
    "engg_c": ("ENGINEERING", "Sr.DEN/C"),
    "engg_s": ("ENGINEERING", "Sr.DEN/S"),
    "engg_track": ("ENGINEERING", "DEN/TRACK"),
    "engg_full": ("ENGINEERING", None),
    "elect_g": ("ELECT/G", None),
    "elect_tro": ("ELECT/TRO", None),
    "elect_trd": ("ELECT/TRD", None),
    "operating": ("OPTG", None),
    "optg": ("OPTG", None),
    "commercial": ("COMMERCIAL", None),
    "mechanical": ("MECHANICAL", None),
    "snt": ("SIGNAL & TELECOM", None),
...
# =============================================================================
# PART A – GENERAL ANALYSIS (PIL)  [original master_code.py logic]
# =============================================================================
...
def get_dashboard_details(department: str, target_den=None):
    if department == "ENGINEERING":
        if target_den is None:
            name = "ENGG_General_Analysis"
            title = " (ENGINEERING DEPARTMENT)"
        else:
            name = f"ENGG_{clean_file_name(target_den)}_Analysis"
            title = f" (ENGINEERING DEPARTMENT - {target_den})"
    else:
        name = f"{clean_file_name(department)}_Analysis"
        title = f" ({DEPARTMENT_DISPLAY_NAMES.get(department, department)})"
    return name, title, f"{name}.png"
...
def centered_text(draw, text, box, fnt, fill, anchor="mm"):
    x1, y1, x2, y2 = box
    draw.text(((x1 + x2) / 2, (y1 + y2) / 2), str(text), font=fnt, fill=fill, anchor=anchor)


def shadow_box(img, draw, xy, radius=18, fill="white"):
    shadow = Image.new("RGBA", img.size, (0, 0, 0, 0))
    sd = ImageDraw.Draw(shadow)
    x1, y1, x2, y2 = xy
    sd.rounded_rectangle((x1, y1 + 7, x2, y2 + 7), radius=radius, fill=(0, 0, 0, 45))
    shadow = shadow.filter(ImageFilter.GaussianBlur(8))
    img.paste(shadow, (0, 0), shadow)
    draw.rounded_rectangle(xy, radius=radius, fill=fill)


def gear(draw, cx, cy, r=24):
    for i in range(8):
        a = i * math.pi / 4
        x = cx + math.cos(a) * (r + 5)
        y = cy + math.sin(a) * (r + 5)
        draw.rectangle((x - 5, y - 5, x + 5, y + 5), fill="#7e9fd1")
    draw.ellipse((cx - r, cy - r, cx + r, cy + r), fill="#8fb0df", outline="#173d78", width=4)
    draw.ellipse((cx - 8, cy - 8, cx + 8, cy + 8), fill="#e9eef6", outline="#173d78", width=3)


def clipboard_icon(draw, cx, cy):
    draw.rounded_rectangle((cx - 27, cy - 34, cx + 27, cy + 37), radius=5,
                           fill="#f7f7f7", outline="#5a3c9b", width=4)
    draw.rounded_rectangle((cx - 12, cy - 43, cx + 12, cy - 28), radius=5,
                           fill="#f0a93b", outline="#704a12", width=3)
    for yy in (-16, 0, 16):
        draw.line((cx - 14, cy + yy, cx - 6, cy + yy + 7), fill="#2f8f53", width=4)
        draw.line((cx - 3, cy + yy + 7, cx + 16, cy + yy - 6), fill="#2f8f53", width=4)
    draw.ellipse((cx + 15, cy + 13, cx + 52, cy + 50), fill="#1f9d55", outline="#155b35", width=3)
    draw.line((cx + 25, cy + 31, cx + 32, cy + 38), fill="white", width=5)
    draw.line((cx + 32, cy + 38, cx + 45, cy + 22), fill="white", width=5)


def helmet_icon(draw, cx, cy):
    draw.pieslice((cx - 31, cy - 31, cx + 31, cy + 31), 180, 360,
                  fill="#f5b51b", outline="#6b4700", width=4)
    draw.rectangle((cx - 31, cy - 3, cx + 31, cy + 8), fill="#f5b51b", outline="#6b4700", width=3)
    draw.rectangle((cx - 7, cy - 32, cx + 7, cy - 8), fill="#ffd15a", outline="#6b4700", width=2)


def speech_icon(draw, cx, cy):
    draw.ellipse((cx - 38, cy - 35, cx + 40, cy + 30), fill="#9fb8ed", outline="#183a72", width=4)
    draw.polygon([(cx - 18, cy + 20), (cx - 30, cy + 48), (cx + 2, cy + 27)],
                 fill="#9fb8ed", outline="#183a72")
    for dx in (-17, 0, 17):
        draw.ellipse((cx + dx - 4, cy - 4, cx + dx + 4, cy + 4), fill="#172d50")


def blueprint_icon(draw, cx, cy):
    draw.rounded_rectangle((cx - 29, cy - 30, cx + 30, cy + 30), radius=3,
                           fill="#1388d0", outline="#0b3d6b", width=3)
    for y in (-12, 8):
        draw.line((cx - 17, cy + y, cx + 18, cy + y), fill="white", width=2)
    for x in (-17, 2):
        draw.line((cx + x, cy - 12, cx + x, cy + 18), fill="white", width=2)
    draw.line((cx - 17, cy + 18, cx + 18, cy - 12), fill="white", width=3)


def load_data(excel_path: Path = EXCEL_FILE) -> pd.DataFrame:
    required = ["Date of Inspection", "Head", "Sub Head", "Status", "Action By"]
    ...
    for sheet in xls.sheet_names:
        try:
            ...
            if all(c in tmp.columns for c in required):
                print(f"Using sheet: {sheet}  ({len(tmp)} rows)")
                return tmp
        except Exception:
            continue
    raise ValueError(f"No sheet contains all required columns: {required}")


def clean_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
    df.columns = df.columns.astype(str).str.strip().str.replace(r"\s+", " ", regex=True)
    df["Head"] = df["Head"].fillna("").astype(str).str.strip().str.upper()
    df["Sub Head"] = df["Sub Head"].fillna("").astype(str).str.strip()
    df["Status"] = df["Status"].fillna("").astype(str).str.strip()
    df["Action By"] = df["Action By"].fillna("").astype(str).str.strip()
...
def get_engineering_data(df: pd.DataFrame, target_den=None) -> pd.DataFrame:
    eng = df[df["Head"] == "ENGINEERING"].copy()
    print(f"  Engineering records before DEN filter: {len(eng)}")
    eng["ACTION_BY_NORMALIZED"] = eng["Action By"].apply(normalize_action_by)
    if target_den is not None:
        target = normalize_action_by(target_den)
        eng = eng[eng["ACTION_BY_NORMALIZED"] == target].copy()
        print(f"  Target DEN: {target_den}  → {len(eng)} records")
    else:
        print(f"  Target DEN: ALL ENGINEERING  → {len(eng)} records")
    return eng[eng["Sub Head"] != ""].copy()


def get_department_data(df: pd.DataFrame, department: str) -> pd.DataFrame:
    head = department.upper().strip()
    dept = df[df["Head"] == head].copy()
    if dept.empty and head == "OPTG":
        dept = df[df["Head"].str.contains("OPTG|OPERATING", regex=True, na=False)].copy()
    if dept.empty and "SIGNAL" in head:
        dept = df[df["Head"].str.contains("SIGNAL|S&T|SNT|TELECOM", regex=True, na=False)].copy()
    return dept[dept["Sub Head"] != ""].copy()


def calculate_categories(department_df: pd.DataFrame) -> list:
    if department_df.empty:
        return []
    subheads = (
        department_df["Sub Head"].dropna().astype(str).str.strip().unique()
    )
    subheads = [s for s in subheads if s]
    categories = []
    for sh in subheads:
        sub = department_df[department_df["Sub Head"] == sh]
        total = len(sub)
        resolved = int((sub["STATUS_GROUP"] == "Resolved").sum())
        pending = int((sub["STATUS_GROUP"] == "Pending").sum())
        no_response = int((sub["STATUS_GROUP"] == "No Response").sum())
...
def create_dashboard(
    ...
    categories = calculate_categories(department_df)
    total_logged = len(department_df)
    total_resolved = int((department_df["STATUS_GROUP"] == "Resolved").sum())
    total_pending = int((department_df["STATUS_GROUP"] == "Pending").sum())
    total_no_response = int((department_df["STATUS_GROUP"] == "No Response").sum())

    dash_name, title_dept, out_name = get_dashboard_details(department, target_den)
    output_file = OUTPUT_FOLDER / out_name

    n_rows = max(len(categories), 1)
    H = 900 if n_rows <= 5 else 900 + (n_rows - 5) * 55

    img = Image.new("RGB", (W, H), "#e9eef6")
    pixels = img.load()
    for y in range(H):
        t = y / H
        r = int(235 - 22 * t)
    ...
    for x in range(-300, W + 400, 220):
        draw.line((x, 0, x + 420, H), fill=(213, 222, 235), width=2)
        draw.line((x + 100, 0, x - 320, H), fill=(225, 231, 241), width=1)
    ...
    if w1 + w2 > W - 80:
        ...
        f_title2 = get_font(32, True)
        bbox1 = draw.textbbox((0, 0), title1, font=f_title)
        bbox2 = draw.textbbox((0, 0), title2, font=f_title2)
        w1 = bbox1[2] - bbox1[0]
        w2 = bbox2[2] - bbox2[0]
    start_x = (W - (w1 + w2)) / 2
    draw.text((start_x, 55), title1, font=f_title, fill="#151515", anchor="lm")
    draw.text((start_x + w1, 55), title2, font=f_title2, fill="#1473b9", anchor="lm")
    centered_text(draw, period_text, (0, 91, W, 148), get_font(25, True), "#171717")

    cards = [
        (28, 157, 390, 316, "TOTAL LOGGED", "DEFICIENCIES", total_logged, "#1554a0"),
        (398, 157, 760, 316, "DEFICIENCIES", "RESOLVED", total_resolved, "#159447"),
        (768, 157, 1240, 316, "PENDING", "DEFICIENCIES", total_pending, "#e02020"),
        (1248, 157, 1572, 316, "NO RESPONSE", "", total_no_response, "#ed8c00"),
    ]
    for x1, y1, x2, y2, lab1, lab2, val, col in cards:
        shadow_box(img, draw, (x1, y1, x2, y2), radius=16, fill="#ffffff")
        centered_text(draw, lab1, (x1 + 105, y1 + 28, x2 - 15, y1 + 70),
                      get_font(23, True), "#111111")
        if lab2:
            centered_text(draw, lab2, (x1 + 105, y1 + 65, x2 - 15, y1 + 107),
                          get_font(23, True), "#111111")
        centered_text(draw, str(val), (x1 + 105, y1 + 108, x2 - 15, y2 - 18),
                      get_font(50, True), col)

    gear(draw, 73, 230, 21)
    gear(draw, 90, 213, 11)
    clipboard_icon(draw, 475, 235)
    helmet_icon(draw, 915, 235)
    speech_icon(draw, 1285, 235)

    main_bottom = H - 35
    shadow_box(img, draw, (28, 335, 1572, main_bottom), radius=14, fill="#ffffff")
    blueprint_icon(draw, 69, 388)
    centered_text(draw, "DEFICIENCY RESOLUTION BREAKDOWN BY SUB-HEAD",
                  (220, 350, 1390, 425), get_font(35, True), "#101010")

    bar_x1, bar_x2 = 365, 1125
    bar_width = bar_x2 - bar_x1
    table_x1, table_x2 = 1200, 1560
    draw.line((1180, 430, 1180, main_bottom - 15), fill="#999999", width=2)

    chart_top = 460
    chart_bottom = main_bottom - 25
    avail_h = chart_bottom - chart_top
    row_spacing = max(48, min(avail_h / n_rows if n_rows else 60, 82))
    bar_h = min(55, max(34, row_spacing - 18))

    th_y, th_h = 438, 52
    draw.rectangle((table_x1, th_y, table_x2, th_y + th_h),
                   fill="#f9f9f9", outline="#777777", width=2)
    draw.line((1450, th_y, 1450, th_y + th_h), fill="#777777", width=1)
    centered_text(draw, "Sub Head", (table_x1, th_y, 1450, th_y + th_h),
                  get_font(19, True), "#111111")
    centered_text(draw, "Total Count", (1450, th_y, table_x2, th_y + th_h),
                  get_font(19, True), "#111111")

    table_start = th_y + th_h
    table_avail = chart_bottom - table_start
    table_row_h = max(40, min(table_avail / (len(categories) + 1) if categories else 50, 70))
    ...
    for idx, item in enumerate(categories):
        ...
        no_resp = item["no_response"]
        pct = item["percentage"]
        cy = chart_top + idx * row_spacing + row_spacing / 2
        fsize = 17 if len(name) > 30 else (19 if len(name) > 24 else 21)
        centered_text(draw, name, (45, cy - bar_h / 2, 345, cy + bar_h / 2),
                      get_font(fsize), "#202020")
        draw.rectangle((bar_x1, cy - bar_h / 2, bar_x2, cy + bar_h / 2), fill="#eeeeee")
        if total > 0:
            rw = (resolved / total) * bar_width
            pw = (pending / total) * bar_width
            nw = (no_resp / total) * bar_width
            if resolved > 0:
        ...
        tfs = 13 if len(name) > 28 else (14 if len(name) > 22 else 15)
        centered_text(draw, name, (table_x1 + 5, ty, 1445, ty + table_row_h),
                      get_font(tfs), "#222222")
        centered_text(draw, str(total), (1450, ty, table_x2, ty + table_row_h),
                      get_font(18, True), "#111111")

    total_y = table_start + len(categories) * table_row_h
    draw.rectangle((table_x1, total_y, table_x2, total_y + table_row_h),
                   fill="#f1f4f8", outline="#555555", width=2)
    centered_text(draw, "Total", (table_x1, total_y, 1450, total_y + table_row_h),
                  get_font(18, True), "#111111")
    centered_text(draw, str(total_logged), (1450, total_y, table_x2, total_y + table_row_h),
                  get_font(20, True), "#111111")
    centered_text(draw, "SUR DIVN, CR", (0, H - 28, W, H), get_font(20, True), "#202020")

    img.save(output_file, quality=95)
    print(f"  ✓ {dash_name}  Sub-heads:{len(categories)} Logged:{total_logged} → {output_file}")
...
def _get_df(excel: Optional[Path] = None) -> pd.DataFrame:
    global _cached_df
    if _cached_df is None or excel is not None:
        path = Path(excel) if excel else EXCEL_FILE
        _cached_df = clean_dataframe(load_data(path))
    return _cached_df
...
def generate_general(
    ...
    """
    Generate one general-analysis (PIL) dashboard.

    key_or_head: friendly key (operating, engg, elect_trd, ...)
                 OR Excel Head string (OPTG, ENGINEERING, ...)
    target_den: only for ENGINEERING (None / Sr.DEN/C / ...)
    """
    k = key_or_head.lower().strip()
    if k in KEY_MAP:
        head, den = KEY_MAP[k]
        if target_den is None:
            target_den = den
    else:
        head = key_or_head.upper().strip()

    df = _get_df(excel)
    if head == "ENGINEERING":
        dept_df = get_engineering_data(df, target_den)
    else:
        dept_df = get_department_data(df, head)
        target_den = None

    print(f"\n[GENERAL] {head}" + (f" / {target_den}" if target_den else ""))
    print(f"  Records: {len(dept_df)}")
    return create_dashboard(head, dept_df, target_den, period_text=period_text)


def generate_all_general(excel: Optional[Path] = None) -> List[str]:
    """Generate all 11 general (PIL) dashboards (4 Engg + 7 others)."""
    df = _get_df(excel)
    paths = []
    print("\n" + "=" * 50)
    print("GENERAL – ENGINEERING")
    print("=" * 50)
    for target in ENGINEERING_TARGETS:
        eng_df = get_engineering_data(df, target)
        paths.append(create_dashboard("ENGINEERING", eng_df, target))
    print("\n" + "=" * 50)
    print("GENERAL – OTHER DEPARTMENTS")
    print("=" * 50)
    for dept in OTHER_DEPARTMENTS:
        dept_df = get_department_data(df, dept)
        print(f"\nProcessing: {dept} ({len(dept_df)} records)")
        paths.append(create_dashboard(dept, dept_df, None))
    print(f"\nAll general dashboards saved under {OUTPUT_FOLDER}")
    return paths


# =============================================================================
# PART B – DETAILED DASHBOARDS (matplotlib)  [original combinations.py logic]
# =============================================================================
...
def filter_months(df: pd.DataFrame, months: Optional[Sequence[int]] = None) -> pd.DataFrame:
    """Keep only rows in the reporting months. Drops invalid/missing dates
    so KPI, sub-head totals, and jurisdiction totals stay consistent.
    """
    months = list(months if months is not None else REPORT_MONTHS)
    if "Month" not in df.columns:
        if "Date of Inspection" in df.columns:
            df = df.copy()
            df["Month"] = pd.to_datetime(df["Date of Inspection"], errors="coerce").dt.month
        else:
            return df
    before = len(df)
...
# ---- Paste remaining detailed generators from combinations.py verbatim ----
# Because the file is large, we import the body by reading the original and
# executing only the generator functions after rebinding EXCEL_FILE / OUTPUT.


# ===== DETAILED GENERATORS (verbatim logic from combinations.py) =====
def generate_elect_g(excel: str = EXCEL_FILE) -> str:
    ...
    out = os.path.join(OUTPUT_FOLDER_STR, "ELECT_G_Dashboard_April_May_June_2026.png")
    save_fig(fig, out)
...
def generate_engineering(target_den: Optional[str] = "Sr.DEN/C",
    ...
    if target_den:
        ...
        out = os.path.join(OUTPUT_FOLDER_STR, f"ENGG_Dashboard_{safe_name}.png")
    else:
        out = os.path.join(OUTPUT_FOLDER_STR, "ENGG_Dashboard.png")
    save_fig(fig, out)
...
def generate_elect_trd(excel: str = EXCEL_FILE) -> str:
    ...
    out = os.path.join(OUTPUT_FOLDER_STR, "ELECT_TRD_Safety_Dashboard.png")
    save_fig(fig, out)
...
def generate_mechanical(excel: str = EXCEL_FILE) -> str:
    ...
    out = os.path.join(OUTPUT_FOLDER_STR, "MECHANICAL_Safety_Dashboard.png")
    save_fig(fig, out)
...
def generate_operating(excel: str = EXCEL_FILE) -> str:
    ...
    out = os.path.join(OUTPUT_FOLDER_STR, "OPERATING_Safety_Dashboard.png")
    save_fig(fig, out)
...
def generate_commercial(excel: str = EXCEL_FILE) -> str:
    ...
    out = os.path.join(OUTPUT_FOLDER_STR, "COMMERCIAL_Safety_Dashboard.png")
    save_fig(fig, out)
...
def generate_elect_tro(excel: str = EXCEL_FILE) -> str:
    ...
    out = os.path.join(OUTPUT_FOLDER_STR, "ELECT_TRO_DEDICATED_ANALYSIS.png")
    save_fig(fig, out)
...
def generate_snt(excel: str = EXCEL_FILE) -> str:
    ...
    out = os.path.join(OUTPUT_FOLDER_STR, "SNT_ADSTE_Safety_Dashboard.png")
    save_fig(fig, out)
...
# =============================================================================
# DISPATCHER – call any / all dashboards from BOTH families
# =============================================================================

# Detailed (matplotlib) dashboards – same keys as original combinations.py
...
# General (PIL) keys – prefix with "general_" or use generate_general()
GENERAL_KEYS = {
    "general_engg": "engg",
    "general_engg_c": "engg_c",
    "general_engg_s": "engg_s",
    "general_engg_track": "engg_track",
    "general_engg_full": "engg_full",
    "general_elect_g": "elect_g",
    "general_elect_tro": "elect_tro",
    "general_elect_trd": "elect_trd",
    "general_operating": "operating",
    "general_optg": "optg",
...
def generate_all_detailed(names: Optional[Sequence[str]] = None) -> List[str]:
    """Generate selected (or CONFIG-enabled) detailed dashboards. Returns list of output paths."""
    if names:
        targets = list(names)
    else:
    ...
    for name in ordered:
        print("\n" + "=" * 54)
        print(f"  Generating (detailed): {name}")
        print("=" * 54)
        try:
...
# Alias matching original combinations.py API
generate_all = generate_all_detailed


def generate_everything() -> List[str]:
    """Run ALL detailed (CONFIG flags) + ALL general PIL dashboards."""
    paths = []
    print("\n" + "#" * 60)
    print("#  DETAILED (matplotlib) DASHBOARDS")
    print("#" * 60)
    paths.extend(generate_all_detailed())
    print("\n" + "#" * 60)
    print("#  GENERAL (PIL) DASHBOARDS")
    print("#" * 60)
    paths.extend(generate_all_general())
    print(f"\nDone. {len(paths)} dashboard(s) written under {OUTPUT_FOLDER}")
    return paths


def main(argv: Optional[Sequence[str]] = None) -> None:
    argv = list(argv if argv is not None else sys.argv[1:])
    if not argv or argv[0] in ("all", "--all", "everything"):
        generate_everything()
        return
    if argv[0] in ("detailed", "detail"):
        generate_all_detailed(argv[1:] if len(argv) > 1 else None)
        return
    if argv[0] in ("general", "pil", "summary"):
        if len(argv) == 1:
            generate_all_general()
        else:
            for name in argv[1:]:
                generate_general(name)
        return

    # Mixed keys: detailed keys and/or general_* keys
    detailed_names = []
    general_names = []
    for name in argv:
        k = name.lower().strip()
        if k in GENERAL_KEYS:
            general_names.append(GENERAL_KEYS[k])
        elif k.startswith("general_"):
            general_names.append(k[len("general_"):])
        elif k in DASHBOARDS:
            detailed_names.append(k)
        elif k in KEY_MAP:
            # bare key → treat as general (PIL) when not a detailed dashboard name
            general_names.append(k)
        else:
            print(f"[WARN] Unknown key: {name}")
    if detailed_names:
        generate_all_detailed(detailed_names)
    if general_names:
        for name in general_names:
            generate_general(name)
    if not detailed_names and not general_names:
        print("No valid dashboard keys given. Use: all | detailed | general | <key>...")
