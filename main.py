...
LANGUAGES = {
    ...
    "hi": {
        "title": "🔐 S.A.R.A.L में लॉगिन करें (सुरक्षा असामान्यता रिपोर्ट और कार्य सूची)",
        ...
        "footer": """
        डेटा में किसी भी सुधार के लिए, सुरक्षा विभाग से संपर्क करें: sursafetyposition@gmail.com, संपर्क: रेलवे फोन नं. 55620, सेल: +91 9022507772
        """,
        "forward_instructions": """
...
def filter_dataframe(df: pd.DataFrame, include_index: bool = False) -> pd.DataFrame:
    df_filtered = df.copy()
    for column in st.session_state.get("column_select_filter", []):
...
with tabs[0]:
    ...
    if not filtered.empty:
        ...
        st.session_state.column_select_filter = st.multiselect("Select columns to filter", candidate_columns, key="column_select_filter")
        if st.session_state.column_select_filter:
...
with tabs[1]:
    ...
    if df.empty:
        ...
        st.markdown(f"### {lang['loc pending']}")
