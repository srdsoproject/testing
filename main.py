...
# ---------- CONFIG ----------
st.set_page_config(page_title="S.A.R.A.L - Safety Abnormality Report & Action List", layout="wide", initial_sidebar_state="expanded")

# Custom CSS for attractiveness, mobile-friendliness, and professionalism
st.markdown("""
    <style>
    /* General Styling */
    :root {
...
if not st.session_state.logged_in:
    ...
    st.markdown("### Safety Abnormality Report & Action List")
...
if not user_ack_done:
    ...
    with st.expander("⚠️ Pending Deficiencies Notice", expanded=True):
        ...
        with st.form("ack_form"):
            responder_name = st.text_input("✍️ Your Name", placeholder="Enter your full name")
...
with st.expander("📝 Responses Received", expanded=False):
    try:
...
ALL_LOCATIONS = FOOTPLATE_LIST  # Combined master list
...
def apply_common_filters(df, prefix=""):
    with st.expander("🔍 Apply Additional Filters", expanded=False):
        c1, c2 = st.columns(2)
        c1.multiselect("Inspection By", options=INSPECTION_BY_LIST[1:], 
                       default=st.session_state.get(prefix+"insp", []), key=prefix+"insp")
        c2.multiselect("Action By", options=ACTION_BY_LIST[1:], 
...
st.markdown(
    ...
    <div style="display:flex;align-items:center;justify-content:center;margin-top:10px;margin-bottom:20px;">
        <img src="https://raw.githubusercontent.com/srdsoproject/testing/main/Central%20Railway%20Logo.png"
             height="60" style="margin-right:15px;object-fit:contain;">
        <div>
            <h3 style="margin:0;font-weight:bold;color:var(--primary-color);">
                An initiative by Safety Department, Solapur Division
            </h3>
        </div>
    </div>
    <h1 style="text-align:center;color:var(--primary-color);">📋 S.A.R.A.L</h1>
    <h4 style="text-align:center;font-weight:normal;color:var(--text-color);margin-top:-10px;">
...
with tabs[0]:
    ...
    df["Status"] = df.apply(lambda row: classify_feedback(row["Feedback"], row.get("User Feedback/Remark", "")), axis=1)
    ...
    with st.expander("🔍 Filters", expanded=True):
        c1, c2 = st.columns(2)
        c1.multiselect("Type of Inspection", options=VALID_INSPECTIONS, key="view_type_filter")
        c2.multiselect("Location", options=ALL_LOCATIONS, key="view_location_filter")

        c3, c4 = st.columns(2)
        c3.multiselect("Head", options=HEAD_LIST[1:], key="view_head_filter")
        sub_opts = sorted({s for h in st.session_state.view_head_filter for s in SUBHEAD_LIST.get(h, [])})
        c4.multiselect("Sub Head", options=sub_opts, key="view_sub_filter")
    ...
    st.info(f"🔹 Showing {len(filtered)} record(s) from **{start_date.strftime('%d.%m.%Y')}** to **{end_date.strftime('%d.%m.%Y')}**")
    ...
    if st.session_state.view_head_filter and not filtered.empty:
        ...
        if not subhead_summary.empty:
            ...
            location_display = ", ".join(st.session_state.view_location_filter) or "All Locations"
            fig.text(0.5, 0.02 + 0.015,
                     f"Date Range: {dr} | Department: {heads} | Type: {type_display} | Location: {location_display}",
                     ha='center', fontsize=9, color='gray')
            if st.session_state.view_sub_filter:
                fig.text(0.5, 0.02, f"Sub Head Filter: {', '.join(st.session_state.view_sub_filter)}",
    ...
    # -------------------- EDITOR --------------------
    st.markdown("### ✍️ Edit Feedback/Remarks")
    editable_filtered = filtered.copy()
    if not editable_filtered.empty:
        search_text = st.text_input("🔍 Search Deficiencies", placeholder="Type to search...").strip().lower()
        if search_text:
        ...
        editable_df["Status"] = editable_df["Status"].apply(lambda s: "🔴 Pending" if s == "Pending" else ("🟢 Resolved" if s == "Resolved" else s))
...
with tabs[1]:
    ...
    if df.empty:
        ...
        if not trend.empty:
            ...
            bars = alt.Chart(trend).mark_bar(color=var(--primary-color)).encode(
        ...
        if not pending.empty:
            ...
            dept_counts["color"] = var(--secondary-color)
            dept_counts.loc[:2, "color"] = var(--warning-color)
            ...
            st.warning(f"**Critical Departments:** {critical_text}")
        else:
        ...
        if not combined_counts.empty:
            ...
            top3_critical["color"] = var(--warning-color)
        ...
        selected_locations = st.multiselect("Select Locations", options=pending["Location"].unique(), default=pending["Location"].unique())
        selected_gates = st.multiselect("Select Gates", options=pending["Gate"].unique(), default=pending["Gate"].unique())
        selected_sections = st.multiselect("Select Sections", options=pending["Section"].unique(), default=pending["Section"].unique())
        ...
        if not filtered_pending_final.empty:
            ...
            loc_counts_final["color"] = var(--secondary-color)
            loc_counts_final.loc[:2,"color"] = var(--warning-color)
...
st.markdown("""
**Forwarding Syntax for Departments:**  

- Operating: Pertains to **OPTG**  
- Signal & Telecom: Pertains to **S&T** 
- Commercial: Pertains to **COMMERCIAL**
- ELECT/G: Pertains to **ELECT/G**
- MECHANICAL: Pertains to **MECHANICAL**
- ELECT/TRD: Pertains to **ELECT/TRD**
- ELECT/TRO: Pertains to **ELECT/TRO**
- Engineering South: Pertains to **Sr.DEN/S**
- Engineering North: Pertains to **Sr.DEN/C**
- Finance: Pertains to **FINAINCE**
- Store: Pertains to **STORE**
- Medical: Pertains to **MEDICAL**
- Security: Pertains to **SECURITY**
""")
