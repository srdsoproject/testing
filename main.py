...
with tabs[1]:
    st.markdown("### 📊 Pending Deficiencies Trend by Department (Bar + Overall Trend)")
    ...
    if not df.empty:
        ...
        # ---- Department-wise monthly pending counts ----
        trend = (
            pending
        ...
        if not trend.empty:
            # ===== 1️⃣ Department-wise stacked/grouped bars =====
            dept_bars = (
                alt.Chart(trend)
                .mark_bar()
                .encode(
                    ...
                    color=alt.Color("Head:N", title="Department Head"),
                    tooltip=["yearmonth(Date of Inspection):T", "Head", "PendingCount"],
                )
            )

            # ===== 2️⃣ Overall monthly total for the dotted trend line =====
            overall = (
                trend.groupby("Date of Inspection")["PendingCount"]
                .sum()
                .reset_index()
                .sort_values("Date of Inspection")
                .reset_index(drop=True)
            )
            overall["MonthIndex"] = overall.index  # numeric for regression

            trend_line = (
                alt.Chart(overall)
                .transform_regression("MonthIndex", "PendingCount")
                .mark_line(color="red", strokeDash=[5, 5], strokeWidth=2)
                .encode(
                    x=alt.X("Date of Inspection:T"),
                    y="PendingCount:Q"
                )
            )

            # Combine bars + overall dotted line
            st.altair_chart(dept_bars + trend_line, use_container_width=True)

            # === Department-wise totals below chart ===
            st.markdown("### 🏢 Department-wise Pending Totals")
            dept_totals = (
            ...
            st.dataframe(dept_totals, use_container_width=True)
