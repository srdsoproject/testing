...
editable_filtered = filtered_df.copy()
if not editable_filtered.empty:
    ...
    if "_sheet_row" not in editable_filtered.columns:
        editable_filtered["_sheet_row"] = editable_filtered.index + 2  # excel row (header + 1)
    ...
    # --- Insert Status column (colored) just before User Feedback/Remark ---
    editable_df.insert(
        editable_df.columns.get_loc("User Feedback/Remark"),
        "Status",
        [classify_feedback(r["Feedback"], r["User Feedback/Remark"]) for _, r in editable_df.iterrows()]
    )
    editable_df["Status"] = editable_df["Status"].apply(color_text_status)

    # Keep helper IDs for updating (hidden in grid)
    ...
    # Only “User Feedback/Remark” is editable (large popup for comfort)
    gb.configure_column(
    ...
    if submitted:
        ...
        if not need_cols.issubset(edited_df.columns) or "Feedback" not in editable_filtered.columns:
            ...
            if changed_ids:
                ...
                # ---- Auto-routing to departments by keywords ----
                routing = {
                ...
                for oid in changed_ids:
                    ...
                    # Replace Feedback and clear remark
                ...
                # Persist changes (updates Excel/Sheet)
                update_feedback_column(
                    diffs.reset_index().rename(columns={"index": "_original_sheet_index"})
                )

                # Re-classify Status column
                st.session_state.df["Status"] = st.session_state.df.apply(
