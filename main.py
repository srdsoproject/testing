...
try:
    df_ack = pd.read_excel("responses.xlsx")
    if not df_ack.empty:
        st.dataframe(df_ack)
    else:
...
if st.button("🗑️ Clear All Responses", key="clear_responses_btn"):
    df_ack = pd.DataFrame(columns=["Name"])
    df_ack.to_excel("responses.xlsx", index=False)
    ...
    with st.expander(
        ...
        if st.button("Mark all as Read (Acknowledge)", key="acknowledge_dept_btn"):
            new_entry = {
                ...
                "MarkedAt": pd.Timestamp.now()
            }
            ack_status = pd.concat([ack_status, pd.DataFrame([new_entry])], ignore_index=True)
            ack_status.to_excel("dept_pending_ack.xlsx", index=False)
            st.success("✅ All pending deficiencies marked as read for your department!")
            st.rerun()
